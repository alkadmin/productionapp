...
const IssueProduction = ({ visible, onHide, data, pallet, onSave, parentEntry, chips, chips1, ParentOrKid, Variety }) => {
    ...
    const [batchesSelected, setBatchesSelected] = useState(null);
    ...
    useEffect(() => {
        if (visible && data) {
            ...
            const formatDateToMDY = (date) => {
                ...
                const month = String(d.getMonth() + 1).padStart(2, '0'); // Agrega ceros si es necesario
            ...
            const assignFIFOBatches = async (row, rowIndex) => {
                try {
                    const response = await fetch(`/api/LotsByItem`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (!response.ok) {
                        throw new Error('Error fetching lots from API');
                    }
                    ...
                    let remainingQty = parseFloat(row.real) || 0;
                    const assignedLots = [];
                    ...
                    const formattedLots = lots.map((lot) => ({
                        label: `
                         LOT:${lot.BatchNum}  |
                         QTY:${Number(lot.Quantity).toFixed(3)}  |
                         BB:${formatDateToMDY(lot.ExpDate)}
                        `.trim(),
                    ...
                    for (const lot of lots) {
                        ...
                        const lotQuantity = parseFloat(lot.Quantity) || 0;
                        const qtyToAssign = Math.min(lotQuantity, remainingQty);

                        assignedLots.push({
                            lotNumber: lot.BatchNum,
                            quantity: qtyToAssign.toFixed(2),
                            ExpDate: lot.ExpDate,
                        });
                    ...
                    setBatchesSelected(assignedLots)
                    // Actualizar la fila en el estado del formulario
                    setFormData((prevFormData) => {
                        const updatedRows = [...prevFormData.rows];
                        updatedRows[rowIndex] = {
                            ...updatedRows[rowIndex],
                            lotNumbers: assignedLots,
                            lotOptions: formattedLots,
                            batchAutoAssigned: true,
                        };
                        return { ...prevFormData, rows: updatedRows };
                    });
                } catch (error) {
                    console.error('Error en assignFIFOBatches:', error);
                }
            };

            const initializeFormData = () => {
                if (data) {
                    const formattedData = (Array.isArray(data) ? data : [data]).map((item, index) => {
                        ...
                        const initialRow = {
                            ...
                            lotNumbers: item.Batch === 'Y'
                                ? item.BatchAuto === 'Y'
                                    ? [] // Se llenará automáticamente si BatchAuto es 'Y'
                                    : [{ lotNumber: '', quantity: realQuantity }] // Refleja el valor de 'real' si BatchAuto es 'N'
                                : [],
                            lotOptions: [], // Opciones vacías inicialmente
                            batch: item.Batch,
                        };

                        // Si tiene BatchAuto === 'Y', asignar lotes FIFO automáticamente
                        if (item.Batch === 'Y' && item.BatchAuto === 'Y') {
                            assignFIFOBatches(initialRow, index);
                        }
        ...
        if (rowIndex !== null && rowIndex !== undefined) {
            const updatedRows = formData.rows.map((item, index) => {
                if (index === rowIndex) {
                    ...
                    // Si el campo "real" se cambia y el item tiene BatchAuto === 'Y', reasignar lotes automáticamente
                    if (field === 'real' && item.BatchAuto === 'Y') {
                        const newRealQuantity = parseFloat(e.target.value) || 0;
    
                        // Reasignar lotes basados en ExpDate (FIFO)
                        let remainingQty = newRealQuantity;
    
                        const reassignedLots = (item.lotOptions || [])
                            .sort((a, b) => new Date(a.ExpDate) - new Date(b.ExpDate)) // Ordenar SOLO por ExpDate
                            .reduce((lots, lot) => {
                                if (remainingQty <= 0) return lots;
    
                                const lotQuantity = parseFloat(lot.quantity) || 0;
                                const qtyToAssign = Math.min(lotQuantity, remainingQty);
    
                                lots.push({
                                    lotNumber: lot.value,  // ID del lote
                                    quantity: qtyToAssign.toFixed(2),
                                });
    ...
    const processAndStoreOldestDate = (filteredData) => {
        // Extraer todas las fechas y lotes válidos
        const allBatches = filteredData
            .flatMap((item) =>
                item.lotNumbers?.map((lot) => ({
                    batchNumber: lot.lotNumber,
                    ExpDate: lot.ExpDate
                })) || []
            )
            .filter((lot) => lot.ExpDate); // Filtrar lotes sin fecha
    
        if (allBatches.length === 0) {
            console.warn("No hay lotes con fechas válidas para procesar.");
            return;
        }
    
        // Ordenar por fecha más antigua
        allBatches.sort((a, b) => new Date(a.ExpDate) - new Date(b.ExpDate));
    
        // Obtener el lote más antiguo
        const oldestBatch = allBatches[0];
    
        // Guardar fecha y batch number en localStorage
        localStorage.setItem("fifoInDate", oldestBatch.ExpDate);
        localStorage.setItem("fifoBatchNumber", oldestBatch.batchNumber);
    
        // Mostrar resultados
        console.log("Fechas ordenadas (más viejo al inicio):", allBatches);
        console.log("Fecha más vieja guardada en localStorage:", oldestBatch.ExpDate);
        console.log("Batch más antiguo guardado en localStorage:", oldestBatch.batchNumber);
    };
    ...
    const handleSave = async () => {
        ...
        //     const fifoInDate = assignedLots[0].inDate; // El primer lote asignado
        //    -// const formattedFifoInDate = formatDateToMDY(fifoInDate);
    ...
    const handleAddPallet = async () => {
        ...
        processAndStoreOldestDate(filteredData)
        // 1. Extraer las fechas de inDate y ordenarlas
    
       // console.log(`FIFO InDate saved: ${formattedFifoInDate}`);
        // if (batchesSelected.length > 0) {
        //     const fifoInDate = assignedLots[0].inDate; // El primer lote asignado
        //    -// const formattedFifoInDate = formatDateToMDY(fifoInDate);
            ...
            <div key={rowIndex} className="lote-detail" style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'nowrap' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', width: '300px' }}>
                    {rowData?.lotNumbers?.map((lot, lotIndex) => (
                        <div key={`${rowIndex}-${lotIndex}`} style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'nowrap' }}>
                            <div style={{ flex: '1', width: '320px', minWidth: '320px', maxWidth: '320px' }}>
                                <Dropdown
                                    value={lot.lotNumber}
                                    options={rowData.lotOptions}
                                    onChange={(e) => onLotChange(e, rowIndex, lotIndex, 'lotNumber')}
                                    onFocus={() => fetchLots(rowData.product, rowData.wareHouse, rowIndex)}
                                    placeholder="Select Lot"
                                    disabled={rowData.BatchAuto === 'Y'} // Deshabilitar si es BatchAuto
                                    style={{ width: '100%', fontSize: '12px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
                                />
                            </div>
                            <div style={{ flex: '1', minWidth: '60px', maxWidth: '200px' }}>
                                <InputText
                                    value={lot.quantity}
                                    onChange={(e) => onLotChange(e, rowIndex, lotIndex, 'quantity')}
                                    placeholder="Lot Quantity"
                            ...
                            {showRemoveButton && (
                                <div style={{ flex: '0 0 auto', minWidth: '30px' }}>
                                    <Button
                                        ...
                                        onClick={() => removeLot(rowIndex, lotIndex)}
                                        style={{ color: 'red', fontSize: '12px' }}
                                        disabled={rowData.BatchAuto === 'Y'} // Deshabilitar botón si es BatchAuto
                                    />
                                </div>
                            )}
                        </div>
                ...
                <div style={{ flex: '0 0 auto', minWidth: '30px' }}>
                    <Button
                        ...
                        style={{ fontSize: '12px' }}
                        disabled={rowData.BatchAuto === 'Y'} // Deshabilitar botón de agregar si es BatchAuto
                    />
                </div>
            </div>
        );
