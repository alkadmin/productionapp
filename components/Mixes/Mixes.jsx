...
const Mixes = ({ id, visible, onHide, chips }) => {
    ...
    const fetchMixData = async (id) => {
        try {
            const response = await fetch(chips ? `/api/ProductionMixesChips?id='${id}'` : `/api/ProductionMixes?id=${id}`);
            if (!response.ok) {
    ...
    return (
        <>
            <Toast ref={toast} />
            <Dialog header="Production Mixes" maximizable visible={visible} style={{ width: '85vw' }} footer={footer} onHide={() => { setExpandedRows([]); onHide(); }}>
                {loading ? (
