...
async function handler(req, res) {
    let conn;
    const { method  } = req;
    const { item , warehouse  } = method === "GET" ? req.query : req.body;
    try {
        conn = await (0,_dataBase_dbConnection__WEBPACK_IMPORTED_MODULE_0__/* .getConnection */ .B)(item, warehouse);
        const query = `call CTS_ARTESANO_PROD_GET_LOT_BY_ITEM(${item}, ${warehouse})`;
...
var __webpack_exports__ = __webpack_require__.X(0, [8970], () => (__webpack_exec__(291)));
